#!/bin/bash
##
# Carry out the tar of subfolders of one repo and remove the original subfolder and files
...
usage() {
  ...
  Options:
    -x: originPath: string, obligatory, origin path where are stored the subfolders with
      files.
    -g: userGroup: string, obligatory, user group the files should have.
  Arguments:
    None
  Sbatch parameters:
...
if [[ -z ${originPath} ]] || [[ -z ${userGroup} ]]; then
  error_exit "Line $LINENO: Unexpected number of arguments."
fi

# By precaution, should be in your ~/.bashrc too.
echo "Tar subfolders of $originPath with user group $userGroup..."
alias rm=rm

cd ${originPath}
if [[ ${untar} -eq 0 ]]; then
  subFolders=($(ls -d */ | cut -f1 -d'/'))
  for subFolder in ${subFolders[@]}; do
    chgrp -R ${userGroup} ${subFolder}
    if [[ ! -f ${subFolder}.tar ]]; then
      tar -cvf ${subFolder}.tar ${subFolder} && rm -R ${subFolder}
    else
      tar -uvf ${subFolder}.tar ${subFolder} && rm -R ${subFolder}
    fi
  done
else
  tarFileNames=($(ls *.tar))
  for tarFileName in ${tarFileNames[@]}; do
    subFolder=${tarFileName/\.tar/}
    #if [[ ! -d ${subFolder} ]]; then
      tar xvf $tarFileName --one-top-level
      rm $tarFileName
    #fi
  done
fi
