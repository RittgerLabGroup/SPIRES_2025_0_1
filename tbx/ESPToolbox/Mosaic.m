...
%}
        ...
        function writeSpiresData(obj, waterYearDate)
            ...
            fprintf(['%s: Starting mosaic .mat generation/writing for ', ...
                'region %s, date %s...\n'], ...
                mfilename(), obj.region.name, waterYearDate.toChar());
            varConf = espEnv.myConf.variable( ...
                espEnv.myConf.variable.write_mosaics == 1, :);
            spiresVarConf = varConf(cell2mat(cellfun(@(x) ~isempty(x), ...
                varConf.spires_output_name, UniformOutput = false)), :);
            % solVarNames = {'solar_zenith', 'solar_azimuth'};
            varMetadataNames = {'type', 'units', 'multiplicator_for_mosaics', ...
                'divisor', 'min', 'max', 'nodata_value'};
            objectName = obj.region.name;
...
%}                
                ...
                for varIdx = 1:size(spiresVarConf, 1)
                    varName = spiresVarConf.output_name{varIdx};
                    ...
                    varData2.([varName '_type']) = ...
                        spiresVarConf.type{varIdx};
                    varData2.([varName '_units']) = ...
                        spiresVarConf.units_in_mosaics{varIdx};
                    varData2.([varName '_multiplicator_for_mosaics']) = ...
                        spiresVarConf.multiplicator_for_mosaics(varIdx);
                    varData2.([varName '_divisor']) = spiresVarConf.divisor(varIdx);
                    varData2.([varName '_min']) = spiresVarConf.min(varIdx) * ...
                        spiresVarConf.divisor(varIdx);
                    varData2.([varName '_max']) = spiresVarConf.max(varIdx) * ...
                        spiresVarConf.divisor(varIdx);
                    varData2.([varName '_nodata_value']) = ...
                        spiresVarConf.nodata_value(varIdx);
                end
                Tools.parforSaveFieldsOfStructInFile(outFilePath, ...
                    varData2, appendFlag);
                appendFlag = 'append';
