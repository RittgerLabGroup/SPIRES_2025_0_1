function out=run_spires_landsat(r0dir,rdir,demfile,Ffile,shade,tolval,...
    ...
    el_cutoff,subset,varargin, region, outputFilePath)
    ...
    % region: Regions obj. Whatever region to make work AlbedoForcingCalculator class 20240625.
    % outputFilePath: char. FilePath where the data are saved.
    ...
    % 20240625 Variables format/name and type/nodata saved in files.
    ...
    % 20240625 addition of slope / aspect for slope albedo
    [slopeAngle,aspectAngle] = topographicSlope(dem.Z,dem.hdr.RasterReference);
    %mu0 = cosd(solarZ);
    %mu0(isnan(solarZ)) = NaN;
    mu=sunslope(cosd(solarZ),phi0,slopeAngle,aspectAngle);
    % 20240625
        
    if topocorrect

        % [slopeAngle,aspectAngle] = topographicSlope(dem.Z,dem.hdr.RasterReference); % 20240625 
        % mu=sunslope(cosd(solarZ),phi0,slopeAngle,aspectAngle); % 20240625 
    ...
    out.cc=A.cc;
    % out.mu0=cosd(solarZ); % 20240625

    out.hdr=dem.hdr;
    out.R=R.bands;%REMOVE LATER

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % 20240625 Add albedo, radiative forcing, deltavis
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    saveVariableForSpiresLandsat20240625(outputFilePath, out.fsca_raw, 1, '-new'); % gap_viewable_snow_fraction_s
    saveVariableForSpiresLandsat20240625(outputFilePath, out.fsca, 2, '-append'); % gap_snow_fraction_s
    saveVariableForSpiresLandsat20240625(outputFilePath, out.fshade, 3, '-append'); % gap_shade_fraction_s
    saveVariableForSpiresLandsat20240625(outputFilePath, out.grainradius, 4, '-append'); % gap_grain_size_s
    saveVariableForSpiresLandsat20240625(outputFilePath, out.dust, 5, '-append'); % gap_dust_concentration_s
    saveVariableForSpiresLandsat20240625(outputFilePath, solarZ, 10, '-append'); % solar_zenith
    saveVariableForSpiresLandsat20240625(outputFilePath, phi0, 11, '-append'); % solar_azimuth
    out.fsca_raw = [];
    out.fsca = [];
    out.fshade = [];
    ...
    indicesForNotNaN = find(~isnan(out.grainradius) & ...
        ~isnan(out.dust)); %& ...
        % ~isnan(solarZ)); solarZ cannot be NaN for landsat because only one value.
    ...
    if numel(indicesForNotNaN) ~= 0
        ...
        [albedo, deltavis, radiativeForcing] = albedoForcingCalculator.getFromLookup(out.grainradius(indicesForNotNaN), ...
            out.dust(indicesForNotNaN), solarZmat(indicesForNotNaN));
    ...
    saveVariableForSpiresLandsat20240625(outputFilePath, out.albedo_s, 6, '-append');
    saveVariableForSpiresLandsat20240625(outputFilePath, out.radiative_forcing_s, 7, '-append');
    saveVariableForSpiresLandsat20240625(outputFilePath, out.deltavis_s, 8, '-append');
    out.albedo_s = [];
    out.radiative_forcing_s = [];
    out.deltavis_s = [];
    ...
    saveVariableForSpiresLandsat20240625(outputFilePath, out.albedo_muZ_s, 9, '-append');
%
