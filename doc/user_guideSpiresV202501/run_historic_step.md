...
The historic step process is a production chain which goal is to generate and deliver output historic data for previous time, usually a previous water year, for a set of big regions. For SPIReS v2025.0.1, several big regions can be run, list in `conf/configuration_of_regions.csv`, filtering the column type=bigRegion.
...
At a lower level, the set of jobs submitted for (1) and (2) has as many jobs as the number of indivual tiles form the big Region. For westernUS, there are 5 individual MODIS Tiles (as defined in `conf/configuration_of_regionsSpires.csv`).
...
**Step `spiInges` first waterYear only**. This step is to be run only for the first waterYear of the record. In this project for MODIS, the first waterYear is 2001. The step ingest the data of the months before the first waterYear, but does not run the spectral unmixing of SPIReS on them.
...
**Step `spiInver`**. Crucially, before submitting this step, the user MUST have monitored the outcome of the previous step `mod09ga` and `spiBackg`, as indicated [above](#run-as-a-beginner). This duty is to carry out for each step.
...
*For the Northern Hemisphere for first waterYear*, we also need to apply spectral unmixing on the 3 months before the start of the waterYear, because the temporal interpolation done in the next take the data of these previous months. The commands are:
```bash
bigRegionId=5
confOfMonthId=41
optionForWaterYearDateString=
...
*For the Southern Hemisphere for first waterYear*, the commands are:
```bash
bigRegionId=5
confOfMonthId=41
optionForWaterYearDateString=
...
As said in [Data spaces](run_nrt_pipeline.md#data-spaces), which includes a procedure, it's a good practice for users generating historical data to regularly check their available space and inodes and act accordingly if quotas are close to be reached.

Specifically, SPIReS v2025.0.1 was designed to solve the recurrent issue of corrupted .mat files occuring when a job is killed, wether by the user (command `scancel` [in slurm](install.md#slurm-knowledge)) or by slurm (out of memory or wall-time errors). This was done by replacing the format .mat (used in the version SPIReS v2024.1.0) by .tif, and saving each variable generated by the algorithm in an individual .tif file per tile per day.
